...
class Gateway3(Thread):
    ...
    def deploy(self):
        ...
        while self.devices is None:
            if self._miio_connect():
                print("Connected via MIIO")
                self.devices = self._get_devices1()
                # else:
                #     self._enable_telnet()
            else:
                print("MIIO connect failed; waiting for 30 seconds")
                time.sleep(30)
        print("Got devices {}".format(self.devices))
    ...
    def get_device_prop(self, prop, device):
        for DEVICE in DEVICES:
