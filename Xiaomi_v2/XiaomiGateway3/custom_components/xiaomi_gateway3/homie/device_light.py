#!/usr/bin/env python

from homie.device_base import Device_Base
from homie.node.property.property_battery import Property_Battery
from homie.node.node_integer import Node_Integer

import logging

logger = logging.getLogger(__name__)

class Device_Light(Device_Base):

    def __init__(self, device_id=None, name=None, homie_settings=None, mqtt_settings=None):
        super().__init__ (device_id, name, homie_settings, mqtt_settings)
        self.node = Node_Integer(self,id='light',name='Light',set_value=self.set_value)
        self.add_node(self.node)
        self.battery = Property_Battery(self.node)
        self.node.add_property(self.battery)
        self.start()

    def update_value(self,value):
        self.get_node('light').update_value(value)
        logger.debug('Light Update {}'.format(value))

    def set_value(self,value):#received commands from clients
        # subclass must override and provide logic to set the device
        logger.debug('Light Set {}'.format(value))

    def update_battery(self,battery):
        logger.info ('Updated Battery {}'.format(battery))
        self.battery.value = battery

