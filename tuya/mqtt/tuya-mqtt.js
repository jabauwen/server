...
async function republishDevices() {
    for (let i = 0; i < 2; i++) {
        console.info('Resending device config/state in 30 seconds')
...
const main = async() => {
    ...
    console.info("Read devices file")
    try {
    ...
    console.info("Establishing connection to mqtt client")
    ...
    mqttClient.on('connect', function (err) {
        console.info('Connection established to MQTT server')
    ...
    mqttClient.on('reconnect', function (error) {
        if (mqttClient.connected) {
            console.info('Connection to MQTT server lost. Attempting to reconnect...')
        } else {
            console.info('Unable to connect to MQTT server')
        }
    })

    mqttClient.on('error', function (error) {
        console.info('Unable to connect to MQTT server', error)
    })
    ...
    mqttClient.on('message', function (topic, message) {
        try {
            ...
            if (topic === 'homeassistant/status' || topic === 'hass/status' ) {
                console.info('Home Assistant state topic '+topic+' received message: '+message)
                if (message === 'online') {
                    republishDevices()
                }
            } else if (commandTopic.includes('command')) {
                // If it looks like a valid command topic try to process it
                console.info('Received MQTT message -> ', JSON.stringify({
                    topic: topic,
                    message: message
                }))
